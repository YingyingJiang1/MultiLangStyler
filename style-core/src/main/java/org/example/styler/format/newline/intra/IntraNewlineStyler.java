...
//	private boolean isBreakNode(ExtendContext node, MyParser parser) {
//		int rule = node.getRuleIndex();
//		return rule == parser.getRuleExpression() || rule == parser.getRuleFormalParameter();
//	}
//
//	private void applyOnAST(ExtendContext cur, MyParser parser,
//						  IntraNewlineProperty targetProperty, int targetLen,
//						  int cumulativeLength, int succeedLineNumber) {
//
//		// 否则继续遍历子节点
//		for (int i = 0; i < cur.getChildCount(); i++) {
//			if (cur.getChild(i) instanceof ExtendContext node) {
//				boolean breaked = false;
//				// 判断是否是可换行节点
//				if (isBreakNode(node, parser)) {
//					int nodeLen = node.getText().length();
//					cumulativeLength += nodeLen;
//
//					// 达到阈值时在该节点结束位置换行
//					if (cumulativeLength >= targetLen) {
//						Token stopToken = node.getStop();
//
//						NewlineApplicator.addNewline(stopToken, targetProperty.newlines, parser);
//						++succeedLineNumber;
//
//						// 记录缩进
//						String relativeIndention = targetProperty.getRelativeIndention(succeedLineNumber);
//						Token nextToken = getNextToken(node, parser);
//						if (nextToken instanceof ExtendToken extendToken) {
//							extendToken.addExtraIndention(relativeIndention);
//						}
//
//						// 重置累计长度
//						cumulativeLength = 0;
...
//
//				if (!breaked) {
//					applyOnAST(node, parser, targetProperty, targetLen, cumulativeLength, succeedLineNumber);
//				}
//			}
//
//		}
//	}
//
//	private Token getNextToken(ExtendContext cur, MyParser parser) {
//		ParserRuleContext parent = cur.getParent();
//		while (parent != null && parent.getChildCount() == 1) {
//			cur = (ExtendContext) parent;
//			parent = parent.getParent();
//		}
//
//		if (parent instanceof ExtendContext parentCtx) {
//			int idx = parentCtx.children.indexOf(cur);
//			if (idx < parentCtx.getChildCount() - 1) {
//				if (parentCtx.getChild(idx + 1) instanceof TerminalNode terminalNode) {
//					return terminalNode.getSymbol();
//				} else if (parentCtx.getChild(idx + 1) instanceof ExtendContext extendCtx) {
//					return extendCtx.getStart();
//				}
//			}
//		}
//		return null;
