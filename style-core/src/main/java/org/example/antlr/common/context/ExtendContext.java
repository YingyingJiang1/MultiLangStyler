package org.example.antlr.common.context;
    ...
    public String getFormattedText(MyParser parser) {
        ...
        BiPredicate<Token, Token> isSpaceNecessary = new BiPredicate<Token, Token>() {
            ...
            public boolean test(Token token, Token token2) {
                return (parser.getIdentifier() == token.getType() || parser.isKeyword(token)) &&
                        (parser.getIdentifier() == token2.getType() || parser.isKeyword(token2));
            }
        };
