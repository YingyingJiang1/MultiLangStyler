publicvoid draw(GraphicsHandler graphicsHandler) {
    ...
    if (!decideTurn.isEmpty()) {
        if (selectionText[0] != null &&decideTurn.peek().equals("1")) {
            ...
            selectionText[0].drawWithParsedNewLines(graphicsHandler, 10);int x = fontX;
            for (int i = 0;i <= compiledCount;i++) {
                if (selectionText[i + 1] != null) {
                    selectionText[i + 1].setY(fontBottomY + 40);selectionText[i + 1].setX(x);x += (selectionText[i + 1].getText().length() * 17 + 15);selectionText[i + 1].drawWithParsedNewLines(graphicsHandler, 10);
                }
            }
        }
    }
