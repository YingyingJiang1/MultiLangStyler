...
class Solution {
  public int maxDistToClosest(int[] seats) {
    ...
    for(int i = 0; i<n; i++) {
      if(seats[i] == 1) {
        if(lastOccupiedIndex==-1) maxDistance = i;//-- if the first seat itself is taken
        else maxDistance = Math.max(maxDistance,(i-lastOccupiedIndex)/2);//-- to check the maximum distance between two occupied seats
