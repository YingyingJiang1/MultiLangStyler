...
public class ExtendContext extends ParserRuleContext {
  ...
  public void updateHierarchy() {
    ...
    int parentRule = parent.getRuleIndex(), rule = getRuleIndex();
    boolean addHierarchy = AntlrHelper.isChangeHierarchyRule(parentRule) ||
        parentRule == JavaParser.RULE_switchBlockStatementGroup && rule != JavaParser.RULE_switchLabel ||
        AntlrHelper.isBlockStmt(parentRule) && AntlrHelper.isStmt(this) && rule != JavaParser.RULE_block;
    if(addHierarchy) {
  ...
  public void addVws(int insertionPoint, int amount) {
    String newlineStr = StringUtils.repeat(System.lineSeparator(), amount);
    TerminalNode t = new TerminalNodeImpl(new ExtendToken(JavaParser.VWS, newlineStr));
    t.setParent(this);
  ...
  * */
  ...
  public void expandChildren() {
    expandChildIf(root -> !(root instanceof JavaParser.AnnotationContext || root instanceof JavaParser.AnnotationListContext));
  }
  ...
  public void deleteStatementCtx() {
    for(int i = 0; i < getChildCount(); ++i) {
      if (getChild(i) instanceof JavaParser.StatementContext stmtCtx) {
  ...
  private boolean isVws(ParseTree parseTree) {
    return parseTree instanceof TerminalNode && ((TerminalNode) parseTree).getSymbol().getType() == JavaParser.VWS;
  }

  public void updateStartToken() {
    if(!children.isEmpty()) {
      int i = 0;
      while(i < children.size() && isVws(children.get(i))) {
        ++i;
      }
      if (i < children.size()) {
        ParseTree startTree = children.get(i);
  ...
  public void updateStopToken() {
    if(!children.isEmpty()) {
      ...
      while(i >= 0 && isVws(children.get(i))) {
        --i;
      }
      if (i >= 0) {
        ParseTree stopTree = children.get(i);
