...
public class SpotDetectorListener extends JavaParserBaseListener {
    ...
    private Map<Integer, Integer> spots;
	private MyParser parser;

    public SpotDetectorListener(Map<Integer, Integer> spots, MyParser parser) {
        this.spots = spots;
        this.parser = parser;
    }
    ...
    private void detect(ExtendContext ctx) {
        ...
        if (structureStyler.isRelevant(ctx, null, parser)) {
            ...
            int structureIndex = structureStyler.extractStructureId(ctx, parser);
            if (structureIndex != -1) {
                spots.put(ctx.getStart().getTokenIndex(), structureIndex);
            }
        }
    }
