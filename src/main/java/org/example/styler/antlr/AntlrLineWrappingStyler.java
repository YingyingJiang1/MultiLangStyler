 ...
 */
public class AntlrLineWrappingStyler extends AntlrConcreteStylerBase {
  int start = 0;
  ...
  public int extractStyle(List<Token> tokens, int curIndex, ProgramStyle programStyle) {
    checkFileChange(tokens, curIndex, programStyle);
  ...
  private void checkFileChange(List<Token> tokens, int curIndex, ProgramStyle programStyle) {
    if (saveTokens == null) {
      saveTokens = tokens;
      return;
    }
