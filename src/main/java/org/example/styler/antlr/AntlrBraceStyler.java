...
public class AntlrBraceStyler extends AntlrConcreteStylerBase {
  ...
  public AntlrBraceStyler() {
  }

  public AntlrBraceStyler(boolean enableExtraction, boolean enableApplication) {
    super(enableExtraction, enableApplication);
  }

  @Override
  public ExtendContext applyStyle(ExtendContext ctx, ProgramStyle programStyle) {
    applySingleLineBlockStyle(ctx, programStyle);
  ...
  public void extractStyle(ExtendContext ctx, ProgramStyle programStyle) {
    extractSingleLineBlockStyle(ctx, programStyle);
    List<ExtendContext> blocks = getAllBlocks(ctx);
    ...
    for (int i = 0; i < blocks.size(); ++i) {
      try {
        ...
        if (isNotMultiBlockStmt || block != ctx.getLastContextChild()) {
          extractBraceInfo(blockType, block, programStyle);
        }
   ...
   * @param programStyle
   */
  private void applySingleLineBlockStyle(ExtendContext ctx, ProgramStyle programStyle) {
  ...
  private void extractSingleLineBlockStyle(ExtendContext ctx, ProgramStyle programStyle) {
    if(!AntlrHelper.isBraceOptionalBlocks(ctx.getRuleIndex())) {
      return;
    }
    FormatStyle formatStyle = (FormatStyle) programStyle.getStyle(ProgramStyle.FORMAT);
   ...
   */
  private void addIndentionForCaseGroup(ExtendContext ctx, ProgramStyle programStyle) {
  ...
  private void extractBraceInfo(BraceInfo.TypeEnum blockType, ExtendContext ctx, ProgramStyle programStyle) {
    FormatStyle formatStyle = (FormatStyle) programStyle.getStyle(ProgramStyle.FORMAT);
    int stmtNum = 0;
