...
import org.example.analysis.StyleType.NamingFormat;
...
public class NamingFormatExtractor extends StyleFeatureExtractor {
    @Override
    public void toFeatureVector(Style style, Map<String, StyleVector> st2svMap) {
        ...
        for (StyleRule rule : style.getRules()) {
            ...
            && rule.getStyleProperty() instanceof NamingFormatProperty property) {
                ...
                mapAttrValue.addValue(NamingFormat.caseFormatAttr, new StringAttrValue(property.caseFormat.name()));
                mapAttrValue.addValue(NamingFormat.maxLengthAttr, new DoubleAttrValue(property.maxLength));
                sv.addAttrValue(context.symbolType.name(), mapAttrValue);
            }
        }

        st2svMap.put(NamingFormat.styleType, sv);
