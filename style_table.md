...
| token       | Numerical literals                               | Use hard-code literals(except -1,0,1) or  constant declarations. |         | oracle            |          |
...
| token       | Modifiers order                                  | Order of all modifiers                                       |         | google convention |          |
...
| statement   | Assignment statement preferences                 | Assignment statement or compound assignment statement.       |         |                   |          |
| statement   | Increment/decrement preferences                  | prefix or suffix, assignment or inc/dec                      |         | 1                 | :bug:    |
| statement   | Continuous logic and conditions preferences      | Nested if                                                    |         | 1,2,3,8           |          |
| statement   | Continuous logic or conditions preferences       |                                                              |         | 1,2,3,8           |          |
...
| statement   | Return statements                                | Whether a `return` statement with a value uses parentheses   |         | oracle            |          |
| block       | Check then return boolean preferences            |                                                              |         | oracle            |          |
| block       | Check then return preferences                    |                                                              |         | oracle            |          |
| block       | Check then assign preferences                    |                                                              |         | 3                 |          |
...
| expression  | Literal position in bool expression              |                                                              |         |                   |          |
